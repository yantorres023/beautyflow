...
export default async function DashboardPage() {
  ...
  return (
    <>
      ...
      <section className="card booking-link-card"><div><p className="eyebrow">Agendamento online</p><h2>Deixe suas clientes escolherem um horário.</h2><p>Compartilhe este link para receber solicitações sem criar contas para elas. Você confirma o horário pela agenda.</p></div><div className="booking-link-actions"><code className="booking-link-value">/agendar/{context.organization.slug}</code><Link className="button button-secondary" href={`/agendar/${context.organization.slug}`} target="_blank" rel="noreferrer">Abrir página <ArrowUpRight size={16} aria-hidden="true" /></Link></div></section>
    </>
  );
}
